...
const sync = repository => {
  ...
  clone.stderr.on('data', data => {
    console.log(`> ${'Error:'.bold.red} ${data.toString()}`);
  });

  clone.on('exit', code => {
    console.log(`> Configuring repository to omit untracked filed on ${'dotfiles status'.magenta}`);
    ...
    exec(`${GIT} ${GIT_PROXY} config --local status.showUntrackedFiles no`)
      ...
      .then(status => {
        ...
        if (conflictingFiles.length) {
          ...
          return Promise.all(promises).then(() => {
            console.log(`> The following were backed up to ${backupDir.bold.blue}`);
            conflictingFiles.forEach(file => console.log(`  - ${path.resolve(HOME_DIR, file)}`));
            console.log('');
          });
        } else {
          // otherwise, proceed
      ...
      .then(() => {
        ...
        const checkout = spawn(GIT, checkoutCommand.split(' '));
        ...
        checkout.on('exit', code => {
          if (!code) {
            ...
            console.log(' *              Repository Configured!           *'.green.bold);
            console.log(' *                                               *'.green.bold);
            console.log(' *************************************************\n'.green.bold);
            ...
            console.log(`eg: ${'dotfiles add ~/.vimrc'.italic}\n`);
          }
        });
      })
